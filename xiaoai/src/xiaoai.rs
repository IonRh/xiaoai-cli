...
impl Xiaoai {
    ...
    pub async fn login(username: &str, password: &str) -> crate::Result<Self> {
        let login_url = Url::parse(LOGIN_SERVER).unwrap();
        ...
        // 初步登录以获取一些认证信息
        let bytes = client
            .get(format!("{LOGIN_SERVER}serviceLogin?sid=micoapi&_json=true"))
            .send()
            .await?
            .error_for_status()?
            .bytes()
        ...
        let bytes = client
            .post(format!("{LOGIN_SERVER}serviceLoginAuth2"))
            .form(&form)
            .send()
            .await?
            .error_for_status()?
    ...
    pub async fn devices(&self) -> crate::Result<Vec<DeviceInfo>> {
        let response = self
            ...
            .await?
            .error_for_code()?;
        let devices = serde_json::from_value(response.data)?;
    ...
    pub async fn get(&self, uri: &str) -> reqwest::Result<Response> {
        let request_id = random_id(30);
        let uri = format!("{uri}&requestId=app_ios_{request_id}");
        let url = self.api_server.join(&uri).unwrap();
